...
pub(crate) fn dispatch_sort<T, F, const W: usize>(slice: &mut [T], key_fn: F, unopt: bool)
where
    F: FnMut(&T) -> RadixKey<W>,
{
    ...
    match slice.len() {
        ...
        len if len <= core::u8::MAX as usize => {
            sort::<u8, T, _, W>(slice, key_fn, unopt);
        }
        #[cfg(not(target_pointer_width = "16"))]
        len if len <= core::u16::MAX as usize => {
            sort::<u16, T, _, W>(slice, key_fn, unopt);
        }
        #[cfg(not(any(target_pointer_width = "16", target_pointer_width = "32")))]
        len if len <= core::u32::MAX as usize => {
            sort::<u32, T, _, W>(slice, key_fn, unopt);
        }
        #[cfg(not(any(
        ...
        len if len <= core::u64::MAX as usize => {
            sort::<u64, T, _, W>(slice, key_fn, unopt);
        }
        _ => {
            sort::<usize, T, _, W>(slice, key_fn, unopt);
        }
    }
}
...
#[inline(never)]
fn sort<O, T, F, const W: usize>(input: &mut [T], mut key_fn: F, unopt: bool)
where
    O: Offset,
    F: FnMut(&T) -> RadixKey<W>,
{
    ...
    let skip_digit = if digit_skip_enabled {
